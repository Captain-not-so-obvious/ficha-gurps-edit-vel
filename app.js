...
function serializeTables() {
  const tablesData = {};
  ['skillsTable', 'handWeaponsTable', 'rangedWeaponsTable', 'advTable', 'disadvTable', 'equipTable'].forEach(tid => {
    ...
    tablesData[tid] = tbody.innerHTML;
  });
  return tablesData;
}
...
// Helper to deserialize tables
function deserializeTables(tablesData) {
  if (!tablesData) return;
  ['skillsTable', 'handWeaponsTable', 'rangedWeaponsTable', 'advTable', 'disadvTable', 'equipTable'].forEach(tid => {
    if (tablesData[tid]) {
      const table = document.getElementById(tid);
      if (table) {
        table.querySelector('tbody').innerHTML = tablesData[tid];
      }
    }
  });
}
...
function importCharacterJSON(event) {
  ...
  reader.onload = (e) => {
    try {
      ...
      if (data['_tables']) {
        deserializeTables(data['_tables']);
      }
